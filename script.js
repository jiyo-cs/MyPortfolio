...
function typeWriter() {
    ...
    const firstChar = currentProfession.charAt(0).toLowerCase();
    const article = ['a', 'e', 'i', 'o', 'u'].includes(firstChar) ? 'an' : 'a';

    if (isDeleting) {
        heroSubtitle.innerHTML = "I'm " + article + " " + currentProfession.substring(0, charIndex - 1) + '<span class="cursor">|</span>';
        charIndex--;
        typingSpeed = 75;
    } else {
        heroSubtitle.innerHTML = "I'm " + article + " " + currentProfession.substring(0, charIndex + 1) + '<span class="cursor">|</span>';
